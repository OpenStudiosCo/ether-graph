(() => {
  ...
  var require_d3_array = __commonJS({
    "node_modules/d3-array/dist/d3-array.js"(exports, module) {
      (function(global, factory) {
        ...
        const ascendingBisect = bisector2(ascending4);
        const bisectRight = ascendingBisect.right;
        const bisectLeft = ascendingBisect.left;
        const bisectCenter = bisector2(number5).center;
        ...
        function sort(values, ...F) {
          ...
          if (f.length === 1 || F.length > 1) {
            ...
            if (F.length > 1) {
              ...
              f = values.map(f);
              index3.sort((i, j) => ascending4(f[i], f[j]));
            }
        ...
        function groupSort(values, reduce2, key) {
          return (reduce2.length === 1 ? sort(rollup(values, reduce2, key), ([ak, av], [bk, bv]) => ascending4(av, bv) || ascending4(ak, bk)) : sort(group(values, key), ([ak, av], [bk, bv]) => reduce2(av, bv) || ascending4(ak, bk))).map(([key2]) => key2);
        }
        var array2 = Array.prototype;
        var slice5 = array2.slice;
        ...
        function ticks3(start2, stop, count3) {
          var reverse2, i = -1, n, ticks4, step;
        ...
        function tickStep5(start2, stop, count3) {
        ...
        function nice2(start2, stop, count3) {
          let prestep;
          while (true) {
            const step = tickIncrement2(start2, stop, count3);
        ...
        function bin() {
          var value = identity5, domain = extent2, threshold3 = sturges;
          function histogram(data) {
            ...
            var xz = domain(values), x06 = xz[0], x12 = xz[1], tz = threshold3(values, x06, x12);
            if (!Array.isArray(tz)) {
              const max6 = x12, tn = +tz;
              if (domain === extent2)
                [x06, x12] = nice2(x06, x12, tn);
              tz = ticks3(x06, x12, tn);
              if (tz[tz.length - 1] >= x12) {
                if (max6 >= x12 && domain === extent2) {
                  const step = tickIncrement2(x06, x12, tn);
                  if (isFinite(step)) {
                    if (step > 0) {
            ...
            for (i = 0; i < n; ++i) {
              x4 = values[i];
              if (x06 <= x4 && x4 <= x12) {
                bins[bisectRight(tz, x4, 0, m2)].push(data[i]);
              }
            }
            return bins;
          }
          ...
          histogram.thresholds = function(_) {
            return arguments.length ? (threshold3 = typeof _ === "function" ? _ : Array.isArray(_) ? constant(slice5.call(_)) : constant(_), histogram) : threshold3;
          };
          return histogram;
        }
        ...
        function quickselect(array3, k2, left2 = 0, right2 = array3.length - 1, compare = ascending4) {
          while (right2 > left2) {
            if (right2 - left2 > 600) {
            ...
            let i = left2;
            let j = right2;
            swap2(array3, left2, k2);
            if (compare(array3[right2], t) > 0)
              swap2(array3, left2, right2);
            while (i < j) {
              swap2(array3, i, j), ++i, --j;
            ...
            if (compare(array3[left2], t) === 0)
              swap2(array3, left2, j);
            else
              ++j, swap2(array3, j, right2);
            if (j <= k2)
              left2 = j + 1;
            if (k2 <= j)
              right2 = j - 1;
        ...
        function swap2(array3, i, j) {
          const t = array3[i];
          array3[i] = array3[j];
          array3[j] = t;
        }
        function quantile3(values, p, valueof) {
          ...
          if (p >= 1)
            return max5(values);
          var n, i = (n - 1) * p, i0 = Math.floor(i), value0 = max5(quickselect(values, i0).subarray(0, i0 + 1)), value1 = min3(values.subarray(i0 + 1));
          return value0 + (value1 - value0) * (i - i0);
        }
        function quantileSorted(values, p, valueof = number5) {
          if (!(n = values.length))
            return;
          if ((p = +p) <= 0 || n < 2)
        ...
        function freedmanDiaconis(values, min4, max6) {
          return Math.ceil((max6 - min4) / (2 * (quantile3(values, 0.75) - quantile3(values, 0.25)) * Math.pow(count2(values), -1 / 3)));
        }
        function scott(values, min4, max6) {
          return Math.ceil((max6 - min4) / (3.5 * deviation(values) * Math.pow(count2(values), -1 / 3)));
        }
        ...
        function median(values, valueof) {
          return quantile3(values, 0.5, valueof);
        }
        ...
        function range6(start2, stop, step) {
          ...
          var i = -1, n = Math.max(0, Math.ceil((stop - start2) / step)) | 0, range7 = new Array(n);
          while (++i < n) {
            range7[i] = start2 + i * step;
          }
          return range7;
        }
        function least(values, compare = ascending4) {
          let min4;
          let defined = false;
          if (compare.length === 1) {
            let minValue;
            for (const element of values) {
              ...
              if (defined ? ascending4(value, minValue) < 0 : ascending4(value, value) === 0) {
                min4 = element;
                minValue = value;
                defined = true;
              }
        ...
        function leastIndex(values, compare = ascending4) {
          if (compare.length === 1)
        ...
        function greatest(values, compare = ascending4) {
          let max6;
          let defined = false;
          if (compare.length === 1) {
            let maxValue;
            for (const element of values) {
              const value = compare(element);
              if (defined ? ascending4(value, maxValue) > 0 : ascending4(value, value) === 0) {
                max6 = element;
                maxValue = value;
                defined = true;
              }
        ...
        function greatestIndex(values, compare = ascending4) {
          if (compare.length === 1)
        ...
        var shuffle2 = shuffler(Math.random);
        ...
        function transpose(matrix) {
          if (!(n = matrix.length))
            return [];
          for (var i = -1, m2 = min3(matrix, length), transpose2 = new Array(m2); ++i < m2; ) {
        ...
        exports2.tickIncrement = tickIncrement2;
        exports2.tickStep = tickStep5;
        exports2.ticks = ticks3;
  ...
  var require_d3_dispatch = __commonJS({
    "node_modules/d3-dispatch/dist/d3-dispatch.js"(exports, module) {
      (function(global, factory) {
        ...
        Dispatch.prototype = dispatch6.prototype = {
  ...
  var require_d3_timer = __commonJS({
    "node_modules/d3-timer/dist/d3-timer.js"(exports, module) {
      (function(global, factory) {
        ...
        "use strict";
        var frame = 0, timeout2 = 0, interval = 0, pokeDelay = 1e3, taskHead, taskTail, clockLast = 0, clockNow = 0, clockSkew = 0, clock = typeof performance === "object" && performance.now ? performance : Date, setFrame = typeof window === "object" && window.requestAnimationFrame ? window.requestAnimationFrame.bind(window) : function(f) {
          setTimeout(f, 17);
        };
        function now2() {
        ...
        Timer.prototype = timer4.prototype = {
          constructor: Timer,
        ...
        function timeout$1(callback, delay, time2) {
          var t = new Timer();
  ...
  var require_d3_polygon = __commonJS({
    "node_modules/d3-polygon/dist/d3-polygon.js"(exports, module) {
      (function(global, factory) {
        typeof exports === "object" && typeof module !== "undefined" ? factory(exports) : typeof define === "function" && define.amd ? define(["exports"], factory) : (global = global || self, factory(global.d3 = global.d3 || {}));
      })(exports, function(exports2) {
        "use strict";
        function area(polygon) {
          var i = -1, n = polygon.length, a3, b = polygon[n - 1], area2 = 0;
          while (++i < n) {
            a3 = b;
            b = polygon[i];
            area2 += a3[1] * b[0] - a3[0] * b[1];
          }
          return area2 / 2;
        }
        ...
        function contains(polygon, point6) {
  ...
  var require_d3_weighted_voronoi = __commonJS({
    "node_modules/d3-weighted-voronoi/build/d3-weighted-voronoi.js"(exports, module) {
      (function(global, factory) {
        typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require_d3_array(), require_d3_polygon()) : typeof define === "function" && define.amd ? define(["exports", "d3-array", "d3-polygon"], factory) : factory(global.d3 = global.d3 || {}, global.d3, global.d3);
      })(exports, function(exports2, d3Array, d3Polygon) {
        "use strict";
        var epsilon8 = 1e-10;
        function epsilonesque(n) {
          return n <= epsilon8 && n >= -epsilon8;
        }
        function dot(v0, v1) {
        ...
        function polygonDirection(polygon) {
          var sign3, crossproduct, p02, p1, p2, v0, v1, i;
          p02 = polygon[polygon.length - 2];
          p1 = polygon[polygon.length - 1];
          p2 = polygon[0];
          v0 = vect(p02, p1);
          v1 = vect(p1, p2);
          crossproduct = calculateCrossproduct(v0, v1);
          sign3 = Math.sign(crossproduct);
          p02 = p1;
          p1 = p2;
          p2 = polygon[1];
          v0 = v1;
          v1 = vect(p1, p2);
          crossproduct = calculateCrossproduct(v0, v1);
          if (Math.sign(crossproduct) !== sign3) {
            return void 0;
          }
        ...
        function Vertex(x4, y4, z, weight, orig, isDummy) {
          this.x = x4;
          this.y = y4;
          this.weight = epsilon8;
          this.index = 0;
        ...
        Vertex.prototype.projectZ = function(x4, y4, weight) {
          return x4 * x4 + y4 * y4 - weight;
        };
        ...
        function Point2D(x4, y4) {
          this.x = x4;
          this.y = y4;
        }
        function Vector(x4, y4, z) {
          this.x = x4;
          this.y = y4;
          this.z = z;
        }
        ...
        function Face(a3, b, c5, orient2) {
          this.conflicts = new ConflictList(true);
          this.verts = [a3, b, c5];
          this.marked = false;
          var t = a3.subtract(b).crossproduct(b.subtract(c5));
        ...
        Face.prototype.orient = function(orient2) {
          if (!(dot(this.normal, orient2) < dot(this.normal, this.verts[0]))) {
        ...
        Face.prototype.conflict = function(v) {
          return dot(this.normal, v) > dot(this.normal, this.verts[0]) + epsilon8;
        };
        ...
        ConvexHull.prototype.removeConflict = function(f) {
          ...
          if (index2 >= this.facets.length || index2 < 0)
            return;
        ...
        function polygonClip(clip, subject) {
          var input, closed = polygonClosed(subject), i = -1, n = clip.length - polygonClosed(clip), j, m2, a3 = clip[n - 1], b, c5, d, intersection;
          while (++i < n) {
            input = subject.slice();
            subject.length = 0;
            b = clip[i];
            c5 = input[(m2 = input.length - closed) - 1];
            j = -1;
            while (++j < m2) {
              d = input[j];
              if (polygonInside(d, a3, b)) {
                if (!polygonInside(c5, a3, b)) {
                  intersection = polygonIntersect(c5, d, a3, b);
                  if (isFinite(intersection[0])) {
                    subject.push(intersection);
                  }
                }
                subject.push(d);
              } else if (polygonInside(c5, a3, b)) {
                intersection = polygonIntersect(c5, d, a3, b);
                if (isFinite(intersection[0])) {
                  subject.push(intersection);
                }
        ...
        function polygonIntersect(c5, d, a3, b) {
          var x12 = c5[0], x32 = a3[0], x21 = d[0] - x12, x43 = b[0] - x32, y12 = c5[1], y32 = a3[1], y21 = d[1] - y12, y43 = b[1] - y32, ua = (x43 * (y12 - y32) - y43 * (x12 - x32)) / (y43 * x21 - x43 * y21);
          return [x12 + ua * x21, y12 + ua * y21];
        }
        function polygonClosed(coordinates2) {
          var a3 = coordinates2[0], b = coordinates2[coordinates2.length - 1];
          return !(a3[0] - b[0] || a3[1] - b[1]);
        }
              ...
              for (var e = 0; e < 3; e++) {
                ...
                if (!verticesVisited[destVertex.index]) {
                  ...
                  for (var j = 0; j < faces.length; j++) {
                    var point6 = faces[j].getDualPoint();
                    var x12 = point6.x;
                    var y12 = point6.y;
                    if (lastX !== null) {
                      dx = lastX - x12;
                    ...
                    if (dx > epsilon8 || dy > epsilon8) {
                      protopoly.push([x12, y12]);
                      lastX = x12;
                      lastY = y12;
                    }
                  }
        ...
        function weightedVoronoi() {
          ...
          function _weightedVoronoi(data) {
            ...
            formatedSites = data.map(function(d) {
              return new Vertex(x4(d), y4(d), null, weight(d), d, false);
            });
          ...
          function boundingSites() {
            var minX, maxX, minY, maxY2, width2, height2, x06, x12, y06, y12, boundingData = [], boundingSites2 = [];
            minX = extent2[0][0];
            maxX = extent2[1][0];
            minY = extent2[0][1];
            maxY2 = extent2[1][1];
            ...
            y06 = minY - height2;
            y12 = maxY2 + height2;
            boundingData[0] = [x06, y06];
            boundingData[1] = [x06, y12];
            boundingData[2] = [x12, y12];
            boundingData[3] = [x12, y06];
            for (var i = 0; i < 4; i++) {
              boundingSites2.push(
                new Vertex(
                  boundingData[i][0],
                  ...
                  null,
                  epsilon8,
                  new Vertex(boundingData[i][0], boundingData[i][1], null, epsilon8, null, true),
                  true
                )
              );
            }
  ...
  var require_d3_voronoi_map = __commonJS({
    "node_modules/d3-voronoi-map/build/d3-voronoi-map.js"(exports, module) {
      (function(global, factory) {
        typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require_d3_polygon(), require_d3_timer(), require_d3_dispatch(), require_d3_weighted_voronoi()) : typeof define === "function" && define.amd ? define(["exports", "d3-polygon", "d3-timer", "d3-dispatch", "d3-weighted-voronoi"], factory) : factory(global.d3 = global.d3 || {}, global.d3, global.d3, global.d3, global.d3);
        ...
        function randomInitialPosition() {
          var clippingPolygon, extent2, minX, maxX, minY, maxY2, dx, dy;
          function _random(d, i, arr, voronoiMapSimulation2) {
            ...
            x4 = minX + dx * voronoiMapSimulation2.prng()();
            y4 = minY + dy * voronoiMapSimulation2.prng()();
            while (!d3Polygon.polygonContains(clippingPolygon, [x4, y4])) {
              x4 = minX + dx * voronoiMapSimulation2.prng()();
              y4 = minY + dy * voronoiMapSimulation2.prng()();
            }
            return [x4, y4];
        ...
        function pie() {
          ...
          function vDistance(vertex, edgeVertex0, edgeVertex1) {
            var x4 = vertex[0], y4 = vertex[1], x12 = edgeVertex0[0], y12 = edgeVertex0[1], x22 = edgeVertex1[0], y22 = edgeVertex1[1];
            var A5 = x4 - x12, B2 = y4 - y12, C2 = x22 - x12, D2 = y22 - y12;
            var dot = A5 * C2 + B2 * D2;
            var len_sq = C2 * C2 + D2 * D2;
            var param = -1;
            if (len_sq != 0)
              param = dot / len_sq;
            var xx, yy;
        ...
        function voronoiMapSimulation(data) {
          ...
          function initialize(data2, simulation2) {
            var maxWeight = data2.reduce(function(max5, d) {
              return Math.max(max5, weight(d));
          ...
          function adaptWeights(polygons2, flickeringMitigationRatio) {
            ...
            for (var i = 0; i < siteCount; i++) {
              ...
              adaptedWeight = Math.max(adaptedWeight, epsilon8);
              ...
              for (var i = 0; i < siteCount; i++) {
                ...
                for (var j = i + 1; j < siteCount; j++) {
                  ...
                  if (sqrD < weightest.weight - lightest.weight) {
                    ...
                    lightest.weight += overweight + epsilon8;
                    fixApplied = true;
                    fixCount++;
                    break;
                  }
    ...
    "node_modules/d3-voronoi-treemap/build/d3-voronoi-treemap.js"(exports, module) {
      (function(global, factory) {
        ...
        function voronoiTreemap3() {
          ...
          function _voronoiTreemap2(rootNode) {
            recurse(clip, rootNode);
          }
          _voronoiTreemap2.convergenceRatio = function(_) {
            if (!arguments.length) {
              return convergenceRatio;
            }
            convergenceRatio = _;
            return _voronoiTreemap2;
          };
          _voronoiTreemap2.maxIterationCount = function(_) {
            if (!arguments.length) {
              return maxIterationCount;
            }
            maxIterationCount = _;
          ...
          _voronoiTreemap2.minWeightRatio = function(_) {
            if (!arguments.length) {
              return minWeightRatio;
            }
            minWeightRatio = _;
          ...
          _voronoiTreemap2.clip = function(_) {
            if (!arguments.length) {
              return clip;
            }
          ...
          _voronoiTreemap2.extent = function(_) {
            if (!arguments.length) {
              return extent2;
            }
          ...
          _voronoiTreemap2.size = function(_) {
            if (!arguments.length) {
              return size;
            }
          ...
          _voronoiTreemap2.prng = function(_) {
            if (!arguments.length) {
              return prng;
            }
            prng = _;
        ...
        exports2.voronoiTreemap = voronoiTreemap3;
        Object.defineProperty(exports2, "__esModule", { value: true });
      });
    }
  });
  ...
  // node_modules/d3/index.js
  __reExport(d3_exports, __toESM(require_d3_array()));
  ...
  function axis(orient2, scale) {
    var tickArguments = [], tickValues = null, tickFormat2 = null, tickSizeInner = 6, tickSizeOuter = 6, tickPadding = 3, offset = typeof window !== "undefined" && window.devicePixelRatio > 1 ? 0 : 0.5, k2 = orient2 === top || orient2 === left ? -1 : 1, x4 = orient2 === left || orient2 === right ? "x" : "y", transform2 = orient2 === top || orient2 === bottom ? translateX : translateY;
    function axis2(context) {
      var values = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain() : tickValues, format2 = tickFormat2 == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : identity_default : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range6 = scale.range(), range0 = +range6[0] + offset, range1 = +range6[range6.length - 1] + offset, position = (scale.bandwidth ? center : number)(scale.copy(), offset), selection2 = context.selection ? context.selection() : context, path2 = selection2.selectAll(".domain").data([null]), tick = selection2.selectAll(".tick").data(values, scale).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
  ...
  var import_d3_dispatch3 = __toESM(require_d3_dispatch());

  // node_modules/d3-drag/src/drag.js
  var import_d3_dispatch = __toESM(require_d3_dispatch());
  ...
  function pointer_default(event, node) {
    ...
    if (node) {
      ...
      if (svg3.createSVGPoint) {
        var point6 = svg3.createSVGPoint();
  ...
  function drag_default() {
    var filter2 = defaultFilter, container = defaultContainer, subject = defaultSubject, touchable = defaultTouchable, gestures = {}, listeners = (0, import_d3_dispatch.dispatch)("start", "drag", "end"), active = 0, mousedownx, mousedowny, mousemoving, touchending, clickDistance2 = 0;
    ...
    function beforestart(that, container2, event, d, identifier, touch) {
      var dispatch6 = listeners.copy(), p = pointer_default(touch || event, container2), dx, dy, s2;
  ...
  function hslConvert(o) {
    ...
    if (o instanceof Hsl)
      return o;
    o = o.rgb();
    var r = o.r / 255, g = o.g / 255, b = o.b / 255, min3 = Math.min(r, g, b), max5 = Math.max(r, g, b), h = NaN, s2 = max5 - min3, l = (max5 + min3) / 2;
    if (s2) {
      if (r === max5)
        h = (g - b) / s2 + (g < b) * 6;
      else if (g === max5)
        ...
        h = (r - g) / s2 + 4;
      s2 /= l < 0.5 ? max5 + min3 : 2 - max5 - min3;
      h *= 60;
    } else {
      s2 = l > 0 && l < 1 ? 0 : h;
    }
  ...
  var import_d3_dispatch2 = __toESM(require_d3_dispatch());
  var import_d3_timer = __toESM(require_d3_timer());
  var emptyOn = (0, import_d3_dispatch2.dispatch)("start", "end", "cancel", "interrupt");
  var emptyTween = [];
  var CREATED = 0;
  var SCHEDULED = 1;
  var STARTING = 2;
  ...
  function create(node, id2, self2) {
    ...
    schedules[id2] = self2;
    self2.timer = (0, import_d3_timer.timer)(schedule, 0, self2.time);
    ...
    function start2(elapsed) {
      ...
      for (i in schedules) {
        ...
        if (o.name !== self2.name)
          continue;
        if (o.state === STARTED)
          return (0, import_d3_timer.timeout)(start2);
        if (o.state === RUNNING) {
          o.state = ENDED;
          o.timer.stop();
      ...
      (0, import_d3_timer.timeout)(function() {
  ...
  function selectAll_default3(select) {
    ...
    for (var groups = this._groups, m2 = groups.length, subgroups = [], parents = [], j = 0; j < m2; ++j) {
      for (var group = groups[j], n = group.length, node, i = 0; i < n; ++i) {
        if (node = group[i]) {
          for (var children2 = select.call(node, node.__data__, i, group), child, inherit2 = get(node, id2), k2 = 0, l = children2.length; k2 < l; ++k2) {
  ...
  var import_d3_timer2 = __toESM(require_d3_timer());
  var defaultTiming = {
    time: null,
    // Set on use.
    delay: 0,
  ...
  function transition_default2(name) {
    ...
    if (name instanceof Transition) {
      ...
      id2 = newId(), (timing = defaultTiming).time = (0, import_d3_timer2.now)(), name = name == null ? null : name + "";
    }
    for (var groups = this._groups, m2 = groups.length, j = 0; j < m2; ++j) {
      for (var group = groups[j], n = group.length, node, i = 0; i < n; ++i) {
        if (node = group[i]) {
  ...
  function brush(dim) {
    var extent2 = defaultExtent, filter2 = defaultFilter2, touchable = defaultTouchable2, keys = true, listeners = (0, import_d3_dispatch3.dispatch)("start", "brush", "end"), handleSize = 6, touchending;
    ...
    function started(event) {
      ...
      function move(event2) {
        ...
        switch (mode) {
          ...
          case MODE_DRAG: {
            if (signX)
              dx = max(W - w0, min(E2 - e0, dx)), w1 = w0 + dx, e1 = e0 + dx;
            if (signY)
          ...
          case MODE_HANDLE: {
            if (points[1]) {
              if (signX)
                w1 = max(W, min(E2, points[0][0])), e1 = max(W, min(E2, points[1][0])), signX = 1;
              if (signY)
                n1 = max(N, min(S, points[0][1])), s1 = max(N, min(S, points[1][1])), signY = 1;
            } else {
              if (signX < 0)
                dx = max(W - w0, min(E2 - w0, dx)), w1 = w0 + dx, e1 = e0;
              else if (signX > 0)
                dx = max(W - e0, min(E2 - e0, dx)), w1 = w0, e1 = e0 + dx;
              if (signY < 0)
          ...
          case MODE_CENTER: {
            if (signX)
              w1 = max(W, min(E2, w0 - dx * signX)), e1 = max(W, min(E2, e0 + dx * signX));
            if (signY)
  ...
  function chord(directed, transpose) {
    ...
    function chord2(matrix) {
      var n = matrix.length, groupSums = new Array(n), groupIndex = range(0, n), chords = new Array(n * n), groups = new Array(n), k2 = 0, dx;
      ...
      dx = k2 ? padAngle : tau2 / n;
      {
        ...
        for (const i of groupIndex) {
          ...
          if (directed) {
            ...
            groups[i] = { index: i, startAngle: x06, endAngle: x4, value: groupSums[i] };
          } else {
  ...
  var import_d3_array = __toESM(require_d3_array());
  ...
  function contours_default() {
    var dx = 1, dy = 1, threshold3 = import_d3_array.thresholdSturges, smooth = smoothLinear;
    function contours(values) {
      var tz = threshold3(values);
      if (!Array.isArray(tz)) {
        var domain = (0, import_d3_array.extent)(values), start2 = domain[0], stop = domain[1];
        tz = (0, import_d3_array.tickStep)(start2, stop, tz);
        tz = (0, import_d3_array.range)(Math.floor(start2 / tz) * tz, Math.floor(stop / tz) * tz, tz);
      } else {
    ...
    function isorings(values, value, callback) {
      ...
      function stitch(line) {
        var start2 = [line[0][0] + x4, line[0][1] + y4], end = [line[1][0] + x4, line[1][1] + y4], startIndex = index2(start2), endIndex = index2(end), f, g;
  ...
  var import_d3_array2 = __toESM(require_d3_array());
  ...
  function density_default() {
    var x4 = defaultX, y4 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m2 = dy + o * 2 >> k2, threshold3 = constant_default6(20);
    function density(data) {
      var values0 = new Float32Array(n * m2), values1 = new Float32Array(n * m2);
      ...
      if (!Array.isArray(tz)) {
        var stop = (0, import_d3_array2.max)(values0);
        tz = (0, import_d3_array2.tickStep)(0, stop, tz);
        tz = (0, import_d3_array2.range)(0, Math.floor(stop / tz) * tz, tz);
        tz.shift();
      }
    ...
    density.thresholds = function(_) {
      return arguments.length ? (threshold3 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default6(slice3.call(_)) : constant_default6(_), density) : threshold3;
    };
  ...
  var Delaunator = class _Delaunator {
    ...
    update() {
      ...
      for (let k2 = 0, xp, yp; k2 < this._ids.length; k2++) {
        ...
        hullSize++;
        let n2 = hullNext[e];
        while (q = hullNext[n2], orient(x4, y4, coords[2 * n2], coords[2 * n2 + 1], coords[2 * q], coords[2 * q + 1])) {
  ...
  // node_modules/d3/index.js
  __reExport(d3_exports, __toESM(require_d3_dispatch()));
  ...
  function pad(value, width2) {
    ...
    return length < width2 ? new Array(width2 - length + 1).join(0) + s2 : s2;
  }
  ...
  function link_default(links) {
    var id2 = index, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count2, bias, random, iterations2 = 1;
    if (links == null)
      links = [];
  ...
  var import_d3_dispatch4 = __toESM(require_d3_dispatch());
  var import_d3_timer3 = __toESM(require_d3_timer());
  ...
  function simulation_default(nodes) {
    var simulation, alpha = 1, alphaMin = 1e-3, alphaDecay = 1 - Math.pow(alphaMin, 1 / 300), alphaTarget = 0, velocityDecay = 0.6, forces = /* @__PURE__ */ new Map(), stepper = (0, import_d3_timer3.timer)(step), event = (0, import_d3_dispatch4.dispatch)("tick", "end"), random = lcg_default();
    if (nodes == null)
      nodes = [];
    function step() {
      tick();
  ...
  function locale_default(locale3) {
    ...
    function newFormat(specifier) {
      ...
      var fill = specifier.fill, align = specifier.align, sign3 = specifier.sign, symbol = specifier.symbol, zero2 = specifier.zero, width2 = specifier.width, comma = specifier.comma, precision = specifier.precision, trim = specifier.trim, type2 = specifier.type;
      if (type2 === "n")
        comma = true, type2 = "g";
  ...
  var import_d3_array3 = __toESM(require_d3_array());
  ...
  // node_modules/d3-geo/src/area.js
  var areaRingSum = new import_d3_array3.Adder();
  var areaSum = new import_d3_array3.Adder();
  var lambda00;
  var phi00;
  var lambda0;
  var cosPhi0;
  ...
  var areaStream = {
    ...
    polygonStart: function() {
      areaRingSum = new import_d3_array3.Adder();
  ...
  function area_default2(object2) {
    areaSum = new import_d3_array3.Adder();
    stream_default(object2, areaStream);
    return areaSum * 2;
  }

  // node_modules/d3-geo/src/bounds.js
  var import_d3_array4 = __toESM(require_d3_array());
  ...
  var boundsStream = {
    ...
    polygonStart: function() {
      ...
      deltaSum = new import_d3_array4.Adder();
  ...
  function rangeContains(range6, x4) {
    return range6[0] <= range6[1] ? range6[0] <= x4 && x4 <= range6[1] : x4 < range6[0] || range6[1] < x4;
  }
  ...
  var import_d3_array5 = __toESM(require_d3_array());
  var W0;
  var W1;
  var X0;
  var Y0;
  ...
  function centroid_default(object2) {
    ...
    X2 = new import_d3_array5.Adder();
    Y2 = new import_d3_array5.Adder();
    Z2 = new import_d3_array5.Adder();
  ...
  var import_d3_array6 = __toESM(require_d3_array());
  ...
  function polygonContains_default(polygon, point6) {
    ...
    var sum2 = new import_d3_array6.Adder();
    if (sinPhi === 1)
    ...
    return (angle2 < -epsilon6 || angle2 < epsilon6 && sum2 < -epsilon22) ^ winding & 1;
  }

  // node_modules/d3-geo/src/clip/index.js
  var import_d3_array7 = __toESM(require_d3_array());
    ...
    return function(sink) {
      ...
      var clip = {
        ...
        polygonEnd: function() {
          ...
          segments = (0, import_d3_array7.merge)(segments);
  ...
  var import_d3_array8 = __toESM(require_d3_array());
  var clipMax = 1e9;
  var clipMin = -clipMax;
  function clipRectangle(x06, y06, x12, y12) {
    ...
    return function(stream) {
      ...
      function polygonEnd() {
        var startInside = polygonInside(), cleanInside = clean && startInside, visible2 = (segments = (0, import_d3_array8.merge)(segments)).length;
  ...
  var import_d3_array9 = __toESM(require_d3_array());
  var lengthSum;
  var lambda03;
  var sinPhi02;
  var cosPhi02;
  ...
  function length_default(object2) {
    lengthSum = new import_d3_array9.Adder();
  ...
  var import_d3_array10 = __toESM(require_d3_array());
  function graticuleX(y06, y12, dy) {
    var y4 = (0, import_d3_array10.range)(y06, y12 - epsilon6, dy).concat(y12);
    return function(x4) {
  ...
  function graticuleY(x06, x12, dx) {
    var x4 = (0, import_d3_array10.range)(x06, x12 - epsilon6, dx).concat(x12);
    return function(y4) {
  ...
  function graticule() {
    ...
    function lines() {
      return (0, import_d3_array10.range)(ceil(X03 / DX) * DX, X13, DX).map(X3).concat((0, import_d3_array10.range)(ceil(Y03 / DY) * DY, Y13, DY).map(Y3)).concat((0, import_d3_array10.range)(ceil(x06 / dx) * dx, x12, dx).filter(function(x5) {
        return abs3(x5 % DX) > epsilon6;
      }).map(x4)).concat((0, import_d3_array10.range)(ceil(y06 / dy) * dy, y12, dy).filter(function(y5) {
  ...
  var import_d3_array11 = __toESM(require_d3_array());
  var areaSum2 = new import_d3_array11.Adder();
  var areaRingSum2 = new import_d3_array11.Adder();
  var x00;
  var y00;
  var x02;
  var y02;
  var areaStream2 = {
    ...
    polygonEnd: function() {
      ...
      areaRingSum2 = new import_d3_array11.Adder();
    },
    result: function() {
      var area = areaSum2 / 2;
      areaSum2 = new import_d3_array11.Adder();
      return area;
    }
  };
  ...
  var import_d3_array12 = __toESM(require_d3_array());
  var lengthSum2 = new import_d3_array12.Adder();
  var lengthRing;
  var x003;
  var y003;
  var x05;
  ...
  var lengthStream2 = {
    ...
    result: function() {
      ...
      lengthSum2 = new import_d3_array12.Adder();
      return length;
    }
  };
  ...
  function fitWidth(projection2, width2, object2) {
    return fit(projection2, function(b) {
      var w = +width2, k2 = w / (b[1][0] - b[0][0]), x4 = (w - k2 * (b[1][0] + b[0][0])) / 2, y4 = -k2 * b[0][1];
      projection2.scale(150 * k2).translate([x4, y4]);
    }, object2);
  }
  function fitHeight(projection2, height2, object2) {
    return fit(projection2, function(b) {
      var h = +height2, k2 = h / (b[1][1] - b[0][1]), x4 = -k2 * b[0][0], y4 = (h - k2 * (b[1][1] + b[0][1])) / 2;
      projection2.scale(150 * k2).translate([x4, y4]);
    }, object2);
  }
  ...
  function packEnclose(circles) {
    if (!(n = (circles = array_default3(circles)).length))
      return 0;
    var a3, b, c5, n, aa, ca, i, j, k2, sj, sk;
  ...
  // node_modules/d3/index.js
  __reExport(d3_exports, __toESM(require_d3_polygon()));
  ...
  var uniform_default = function sourceRandomUniform(source) {
    function randomUniform(min3, max5) {
  ...
  var int_default = function sourceRandomInt(source) {
    function randomInt(min3, max5) {
      if (arguments.length < 2)
        max5 = min3, min3 = 0;
      min3 = Math.floor(min3);
      max5 = Math.floor(max5) - min3;
      return function() {
        return Math.floor(source() * max5 + min3);
      };
    }
  ...
  var import_d3_array13 = __toESM(require_d3_array());
  ...
  function ordinal() {
    var index2 = /* @__PURE__ */ new Map(), domain = [], range6 = [], unknown = implicit;
    function scale(d) {
  ...
  function band() {
    ...
    function rescale() {
      ...
      var values = (0, import_d3_array13.range)(n).map(function(i) {
  ...
  var import_d3_array16 = __toESM(require_d3_array());

  // node_modules/d3-scale/src/continuous.js
  var import_d3_array14 = __toESM(require_d3_array());
  ...
  function polymap(domain, range6, interpolate) {
    ...
    return function(x4) {
      var i2 = (0, import_d3_array14.bisect)(domain, x4, 1, j) - 1;
      return r[i2](d[i2](x4));
    };
  }
  ...
  function transformer2() {
    var domain = unit, range6 = unit, interpolate = value_default, transform2, untransform, unknown, clamp = identity2, piecewise2, output, input;
    function rescale() {
    ...
    function scale(x4) {
      return x4 == null || isNaN(x4 = +x4) ? unknown : (output || (output = piecewise2(domain.map(transform2), range6, interpolate)))(transform2(clamp(x4)));
    }
    scale.invert = function(y4) {
      return clamp(untransform((input || (input = piecewise2(range6, domain.map(transform2), number_default)))(y4)));
    };
    scale.domain = function(_) {
  ...
  var import_d3_array15 = __toESM(require_d3_array());
  function tickFormat(start2, stop, count2, specifier) {
    var step = (0, import_d3_array15.tickStep)(start2, stop, count2), precision;
  ...
  function linearish(scale) {
    ...
    scale.ticks = function(count2) {
      var d = domain();
      return (0, import_d3_array16.ticks)(d[0], d[d.length - 1], count2 == null ? 10 : count2);
    };
    ...
    scale.nice = function(count2) {
      ...
      while (maxIter-- > 0) {
        step = (0, import_d3_array16.tickIncrement)(start2, stop, count2);
        if (step === prestep) {
          d[i0] = start2;
          d[i1] = stop;
          return domain(d);
  ...
  var import_d3_array17 = __toESM(require_d3_array());
  ...
  function loggish(transform2) {
    ...
    scale.ticks = function(count2) {
      ...
      if (!(base % 1) && j - i < n) {
        ...
        if (z.length * 2 < n)
          z = (0, import_d3_array17.ticks)(u, v, n);
      } else {
        z = (0, import_d3_array17.ticks)(i, j, Math.min(j - i, n)).map(pows);
      }
  ...
  function radial() {
    ...
    scale.range = function(_) {
      return arguments.length ? (squared.range((range6 = Array.from(_, number3)).map(square)), scale) : range6.slice();
    };
    scale.rangeRound = function(_) {
  ...
  var import_d3_array18 = __toESM(require_d3_array());
  function quantile() {
    ...
    function rescale() {
      ...
      while (++i < n)
        thresholds[i - 1] = (0, import_d3_array18.quantileSorted)(domain, i / n);
      return scale;
    }
    function scale(x4) {
      return x4 == null || isNaN(x4 = +x4) ? unknown : range6[(0, import_d3_array18.bisect)(thresholds, x4)];
    }
    scale.invertExtent = function(y4) {
      var i = range6.indexOf(y4);
      return i < 0 ? [NaN, NaN] : [
    ...
    scale.domain = function(_) {
      ...
      for (let d of _)
        if (d != null && !isNaN(d = +d))
          domain.push(d);
      domain.sort(import_d3_array18.ascending);
      return rescale();
    };
    scale.range = function(_) {
      return arguments.length ? (range6 = Array.from(_), rescale()) : range6.slice();
    ...
    scale.copy = function() {
      return quantile().domain(domain).range(range6).unknown(unknown);
    };
    return initRange.apply(scale, arguments);
  }

  // node_modules/d3-scale/src/quantize.js
  var import_d3_array19 = __toESM(require_d3_array());
  function quantize() {
    var x06 = 0, x12 = 1, n = 1, domain = [0.5], range6 = [0, 1], unknown;
    function scale(x4) {
      return x4 != null && x4 <= x4 ? range6[(0, import_d3_array19.bisect)(domain, x4, 0, n)] : unknown;
    }
    function rescale() {
      var i = -1;
      domain = new Array(n);
  ...
  var import_d3_array20 = __toESM(require_d3_array());
  function threshold2() {
    var domain = [0.5], range6 = [0, 1], unknown, n = 1;
    function scale(x4) {
      return x4 != null && x4 <= x4 ? range6[(0, import_d3_array20.bisect)(domain, x4, 0, n)] : unknown;
    }
    scale.domain = function(_) {
  ...
  var import_d3_array21 = __toESM(require_d3_array());
  function ticker(year2, month2, week, day2, hour2, minute2) {
    ...
    function ticks3(start2, stop, count2) {
      ...
      const ticks4 = interval ? interval.range(start2, +stop + 1) : [];
      return reverse ? ticks4.reverse() : ticks4;
    }
    function tickInterval(start2, stop, count2) {
      const target = Math.abs(stop - start2) / count2;
      const i = (0, import_d3_array21.bisector)(([, , step2]) => step2).right(tickIntervals, target);
      if (i === tickIntervals.length)
        return year2.every((0, import_d3_array21.tickStep)(start2 / durationYear, stop / durationYear, count2));
      if (i === 0)
        return millisecond_default.every(Math.max((0, import_d3_array21.tickStep)(start2, stop, count2), 1));
  ...
  function pad2(value, fill, width2) {
    ...
    return sign3 + (length < width2 ? new Array(width2 - length + 1).join(fill) + string : string);
  }
  function requote(s2) {
  ...
  function calendar(ticks3, tickInterval, year2, month2, week, day2, hour2, minute2, second2, format2) {
    ...
    scale.ticks = function(interval) {
      var d = domain();
      return ticks3(d[0], d[d.length - 1], interval == null ? 10 : interval);
    };
    scale.tickFormat = function(count2, specifier) {
    ...
    scale.copy = function() {
      return copy(scale, calendar(ticks3, tickInterval, year2, month2, week, day2, hour2, minute2, second2, format2));
    };
    return scale;
  }
  function time() {
  ...
  var import_d3_array22 = __toESM(require_d3_array());
  function sequentialQuantile() {
    var domain = [], interpolator = identity2;
    function scale(x4) {
      if (x4 != null && !isNaN(x4 = +x4))
        return interpolator(((0, import_d3_array22.bisect)(domain, x4, 1) - 1) / (domain.length - 1));
    }
    scale.domain = function(_) {
      if (!arguments.length)
        return domain.slice();
      ...
      for (let d of _)
        if (d != null && !isNaN(d = +d))
          domain.push(d);
      domain.sort(import_d3_array22.ascending);
      return scale;
    };
    scale.interpolator = function(_) {
      return arguments.length ? (interpolator = _, scale) : interpolator;
    ...
    scale.quantiles = function(n) {
      return Array.from({ length: n + 1 }, (_, i) => (0, import_d3_array22.quantile)(domain, i / n));
    };
    scale.copy = function() {
  ...
  function transformer4() {
    var x06 = 0, x12 = 0.5, x22 = 1, s2 = 1, t03, t13, t22, k10, k21, interpolator = identity2, transform2, clamp = false, unknown;
    function scale(x4) {
    ...
    scale.domain = function(_) {
      return arguments.length ? ([x06, x12, x22] = _, t03 = transform2(x06 = +x06), t13 = transform2(x12 = +x12), t22 = transform2(x22 = +x22), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1, scale) : [x06, x12, x22];
    };
    scale.clamp = function(_) {
      return arguments.length ? (clamp = !!_, scale) : clamp;
    };
  ...
  function arc_default() {
    ...
    function arc() {
      ...
      if (!(r1 > epsilon7))
        ...
        var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da, da1 = da, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon7 && (padRadius ? +padRadius.apply(this, arguments) : sqrt3(r0 * r0 + r1 * r1)), rc = min2(abs4(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t03, t13;
        if (rp > epsilon7) {
  ...
  function line_default2(x4, y4) {
    ...
    function line(data) {
      var i, n = (data = array_default4(data)).length, d, defined0 = false, buffer;
      if (context == null)
  ...
  function area_default4(x06, y06, y12) {
    ...
    function area(data) {
      var i, j, k2, n = (data = array_default4(data)).length, d, defined0 = false, buffer, x0z = new Array(n), y0z = new Array(n);
      if (context == null)
        output = curve(buffer = path_default());
      for (i = 0; i <= n; ++i) {
        if (!(i < n && defined(d = data[i], i, data)) === defined0) {
  ...
  function pie_default() {
    var value = identity_default5, sortValues = descending_default, sort = null, startAngle = constant_default10(0), endAngle = constant_default10(tau6), padAngle = constant_default10(0);
    function pie(data) {
      var i, n = (data = array_default4(data)).length, j, k2, sum2 = 0, index2 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(tau6, Math.max(-tau6, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
      for (i = 0; i < n; ++i) {
  ...
  function areaRadial_default() {
    var a3 = area_default4().curve(curveRadialLinear), c5 = a3.curve, x06 = a3.lineX0, x12 = a3.lineX1, y06 = a3.lineY0, y12 = a3.lineY1;
  ...
  function stack_default() {
    ...
    function stack(data) {
      ...
      for (i = 0, oz = array_default4(order(sz)); i < n; ++i) {
        sz[oz[i]].index = i;
      }
      offset(sz, oz);
      return sz;
  ...
  // node_modules/d3/index.js
  __reExport(d3_exports, __toESM(require_d3_timer()));

  // node_modules/d3-zoom/src/zoom.js
  var import_d3_dispatch5 = __toESM(require_d3_dispatch());
  ...
  function zoom_default2() {
    var filter2 = defaultFilter3, extent2 = defaultExtent2, constrain = defaultConstrain, wheelDelta = defaultWheelDelta, touchable = defaultTouchable3, scaleExtent = [0, Infinity], translateExtent = [[-Infinity, -Infinity], [Infinity, Infinity]], duration = 250, interpolate = zoom_default, listeners = (0, import_d3_dispatch5.dispatch)("start", "zoom", "end"), touchstarting, touchfirst, touchending, touchDelay = 500, wheelDelay = 150, clickDistance2 = 0, tapDistance = 10;
    ...
    function dblclicked(event, ...args) {
      if (!filter2.apply(this, arguments))
        return;
      var t03 = this.__zoom, p02 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t03.invert(p02), k1 = t03.k * (event.shiftKey ? 0.5 : 2), t13 = constrain(translate(scale(t03, k1), p02, p1), extent2.apply(this, args), translateExtent);
      noevent_default3(event);
      if (duration > 0)
    ...
    zoom.filter = function(_) {
      return arguments.length ? (filter2 = typeof _ === "function" ? _ : constant_default11(!!_), zoom) : filter2;
    };
  ...
  // src/app.js
  var import_d3_voronoi_treemap2 = __toESM(require_d3_voronoi_treemap());

  // src/app/data.js
  function processCSV(rootData) {
    rootData.forEach(function(csvRow) {
      ...
      let weekNumber2 = new Date(transaction.created).getWeek();
      if (!(weekNumber2 in dataTable)) {
        dataTable[weekNumber2] = [];
        weekNumbers.push(parseInt(weekNumber2));
      }
      dataTable[weekNumber2].push(transaction);
    });
  ...
  var walletId = "0xaaaaaa1111111111111111111111111111111111";
  var _2PI = 2 * Math.PI;
  var _voronoiTreemap = (0, import_d3_voronoi_treemap.voronoiTreemap)();
  var hierarchy2;
  var circlingPolygon;
  var fontScale = linear3();
  var svg2;
  var drawingArea;
  var treemapContainer;
  var svgWidth = 960;
  var svgHeight = 500;
  ...
  function updateGraphs2() {
    ...
    svg2 = select_default2("svg#graph").attr("width", svgWidth).attr("height", svgHeight);
    drawingArea = svg2.append("g").classed("drawingArea", true).attr("transform", "translate(" + [margin.left, margin.top] + ")");
    treemapContainer = drawingArea.append("g").classed("treemap-container", true).attr("transform", "translate(" + treemapCenter + ")");
    treemapContainer.append("path").classed("world", true).attr("transform", "translate(" + [-treemapRadius, -treemapRadius] + ")").attr("d", "M" + circlingPolygon.join(",") + "Z");
    let tempRoot = {
      name: walletId,
      children: []
    };
    ...
    transactions.forEach((transaction2) => {
      if (!(transaction2.currency in displayedCurrencies)) {
        let currency2 = {};
        currency2.code = transaction2.currency;
        currency2.name = transaction2.currency;
        currency2.color = colours[Math.floor(Math.random() * colours.length)];
        currency2.balance = 0;
        currency2.children = [];
        tempRoot.children.push(currency2);
        displayedCurrencies[transaction2.currency] = currency2;
      }
      displayedCurrencies[transaction2.currency].balance += parseFloat(transaction2.amount);
      displayedCurrencies[transaction2.currency].children.push(transaction2);
      let tableRow = document.createElement("tr");
      let fieldId = document.createElement("td");
      fieldId.innerHTML = transaction2.id;
      tableRow.append(fieldId);
      let fieldType = document.createElement("td");
      fieldType.innerHTML = transaction2.type;
      tableRow.append(fieldType);
      let fieldTimestamp = document.createElement("td");
      fieldTimestamp.innerHTML = transaction2.timestamp;
      tableRow.append(fieldTimestamp);
      let fieldCreated = document.createElement("td");
      fieldCreated.innerHTML = transaction2.created;
      tableRow.append(fieldCreated);
      let fieldFrom = document.createElement("td");
      fieldFrom.innerHTML = transaction2.from;
      tableRow.append(fieldFrom);
      let fieldTo = document.createElement("td");
      fieldTo.innerHTML = transaction2.to;
      tableRow.append(fieldTo);
      let fieldAmount = document.createElement("td");
      fieldAmount.innerHTML = transaction2.amount;
      tableRow.append(fieldAmount);
      let fieldCurrency = document.createElement("td");
      fieldCurrency.innerHTML = transaction2.currency;
      tableRow.append(fieldCurrency);
      let fieldPrice = document.createElement("td");
      fieldPrice.innerHTML = transaction2.price;
    ...
    var legendHeight = 13, interLegend = 4, colorWidth = legendHeight * 6;
    var legendContainer = drawingArea.append("g").classed("legend", true).attr("transform", "translate(" + [0, legendsMinY] + ")");
    var legends = legendContainer.selectAll(".legend").data(currencies).enter();
    ...
    legend.append("text").classed("tiny", true).attr("transform", "translate(" + [colorWidth + 5, -2] + ")").text(function(d) {
      return d.name;
    });
    legendContainer.append("text").attr("transform", "translate(" + [0, -currencies.length * (legendHeight + interLegend) - 5] + ")").text("Currencies");
    hierarchy2 = hierarchy(tempRoot).sum(function(d) {
      return d.amount;
    });
    _voronoiTreemap.clip(circlingPolygon)(hierarchy2);
    drawTreemap(hierarchy2, tempRoot);
  }
  function clearGraphs() {
    selectAll_default2("svg#graph > *").remove();
  }
  function drawTreemap(hierarchy3, tempRoot) {
    var leaves = hierarchy3.leaves();
    let currencies = tempRoot.children.reverse();
    var cells = treemapContainer.append("g").classed("cells", true).attr("transform", "translate(" + [-treemapRadius, -treemapRadius] + ")").selectAll(".cell").data(leaves).enter().append("path").classed("cell", true).attr("d", function(d) {
      return "M" + d.polygon.join(",") + "z";
    }).style("fill", function(d) {
      return d.parent.data.color;
    });
    var labels = treemapContainer.append("g").classed("labels", true).attr("transform", "translate(" + [-treemapRadius, -treemapRadius] + ")").selectAll(".label").data(leaves).enter().append("g").classed("label", true).attr("transform", function(d) {
      return "translate(" + [d.polygon.site.x, d.polygon.site.y] + ")";
    }).style("font-size", function(d) {
      return fontScale(d.parent.data.balance);
    });
    labels.append("text").classed("value", true).text(function(d) {
      return d.parent.data.balance;
    });
    labels.append("text").classed("name", true).html(function(d) {
      return d.parent.data.balance < 1 ? d.parent.data.code : d.parent.data.name;
    });
    var hoverers = treemapContainer.append("g").classed("hoverers", true).attr("transform", "translate(" + [-treemapRadius, -treemapRadius] + ")").selectAll(".hoverer").data(leaves).enter().append("path").classed("hoverer", true).attr("d", function(d) {
      return "M" + d.polygon.join(",") + "z";
    });
    hoverers.append("title").text(function(d) {
      return parseFloat(d.value).toFixed(2) + " " + d.parent.data.name;
    });
  }
  function computeCirclingPolygon(radius) {
    var points = 60, increment = _2PI / points, circlingPolygon2 = [];
    for (var a3 = 0, i = 0; i < points; i++, a3 += increment) {
      circlingPolygon2.push(
        [radius + radius * Math.cos(a3), radius + radius * Math.sin(a3)]
      );
    }
    return circlingPolygon2;
  }
  ...
  // src/app.js
  window.updateGraphs = updateGraphs2;
  window.handleTimelineButtonClick = handleTimelineButtonClick;
  window.dataTable = {};
  window.weekNumbers = [];
  csv2("sample-gpt-v2.csv").then(function(rootData) {
    processCSV(rootData);
  });
})();
